...
function RenderEntity2(props: RenderEntityProps<Entity>) {
  const item = props.dataItem as Entity & { currency?: string; jurisdiction: string }
  return <div className="company-ownership-item">{item.name}</div>
}

export default () => {
  const [renderFunction, setRenderFunction] = useState<ComponentType<RenderNodeProps<Entity>>>(
    () => RenderEntity
  )
  ...
  return (
    <>
      <button
        onClick={() =>
          setRenderFunction(() => {
            if (renderFunction === RenderEntity) {
              return RenderEntity2
            } else {
              return RenderEntity
            }
          })
